...
| R4 | Carried forward from 2026-09-05 and unchanged: the two Hub 2.0 drafts on `*.netlify.app` are temporary by design and sit in a shared namespace (**accepted**, revisit at Hub 2.0 cutover); `Strict-Transport-Security` is a Netlify platform default rather than anything in this repository (**accepted**); the runbook publishes the test hostname and `forge` login user (**accepted** — disposable box); staff first names were public at the time (**accepted, by instruction** then; since 1.26.0 no personal name appears anywhere, and the test forbids them — enforced by test). | **Accepted.** |
...
| R6 | Carried forward from the previous pass and unchanged: the runbook publishes the test hostname and `forge` login user (**accepted** — disposable box), and staff first names were public at the time (**accepted, by instruction** then; removed in 1.26.0 — no surnames, enforced by test). | **Accepted.** |
...
| R6 | Staff first names and roles are public. | **Accepted, by instruction** at the time; every personal name was removed in 1.26.0 and the guard now forbids them. No surnames and no personal email addresses — enforced by a test, verified at 0 across all pages. |
